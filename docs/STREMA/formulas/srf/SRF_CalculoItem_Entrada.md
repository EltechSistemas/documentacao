# SRF_CalculoItem_Entrada.md

## 📖 Descrição
Fórmula principal para cálculo de itens em documentos fiscais de entrada, responsável pelo processamento completo de tributos, valores e ajustes fiscais em operações de compra.

## 🎯 Finalidade
Calcular automaticamente valores tributários, conversões de unidades, custos de aquisição e tratamentos fiscais específicos para operações de entrada, garantindo conformidade fiscal e precisão nos cálculos.

## 👥 Público-Alvo
- Departamento Fiscal
- Compras
- Contabilidade
- Almoxarifado/Estoque

## ⚙️ Configuração
**Recursos Necessários:**
- Fórmula `SRF_CalculoItem_Entrada` - Cálculo principal de itens de entrada

**Localização:** `strema/formulas/srf/`

## 📊 Dados e Fontes
**Tabelas Principais:**
- `EAA01` - Documentos fiscais
- `EAA0103` - Itens do documento
- `ABD01` - Tipos de documento (PCD)
- `ABM01` - Cadastro de itens
- `ABM0101` - Configurações do item por empresa
- `ABM12` - Dados fiscais do item

**Entidades Envolvidas:**
- `Eaa0103` - Item do documento
- `Eaa01` - Documento fiscal
- `Abd01` - Tipo de documento (PCD)
- `Abm01` - Item
- `Abm0101` - Configuração empresa-item

## ⚙️ Parâmetros do Processo

| Parâmetro | Tipo | Obrigatório | Descrição |
|-----------|------|-------------|-----------|
| eaa0103 | Eaa0103 | Sim | Item do documento a ser calculado |

## 🔄 Fluxo do Processo

1. **Inicialização e Validações**
   - Obtém item do documento (EAA0103) e documento pai (EAA01)
   - Carrega dados da central (ABB01) e PCD
   - Valida se documento é de entrada
   - Verifica endereço principal da entidade

2. **Carregamento de Dados Complementares**
   - Endereços da entidade e empresa
   - Configurações do item (fiscal, comercial, valores)
   - Dados tributários (CFOP, NCM, CSTs)
   - Fatores de conversão de unidades

3. **Ajustes Automáticos de CFOP**
   - Determinação se operação é intra ou interestadual
   - Ajuste de CFOP baseado no tipo de item
   - Tratamento específico para material de uso e consumo

4. **Cálculos Principais**
   - Valores do item e totais
   - Conversões de quantidades e unidades
   - Tributos (ICMS, IPI, PIS, COFINS)
   - Cálculo de custo de aquisição

## ⚠️ Regras de Negócio

### Validações Críticas
- Item deve existir no documento
- Documento deve ser de entrada (PCD com abd01es ≠ 1)
- Endereço principal da entidade é obrigatório
- Configuração fiscal do item deve existir

### Ajustes Automáticos de CFOP
- **Mercadoria para Revenda (00):** CFOP 1/2102 ou 1/2403 com IVA
- **Matéria-Prima/Embalagem (01/02):** CFOP 1/2101 ou 1/2401 com IVA
- **Material de Uso e Consumo (07):** CFOP 1/2556 com ajustes específicos de CST

### Cálculos Tributários
- **ICMS:** Tratamento completo por CST (00, 10, 20, 30, 40, 41, 50, 51, 60, 70, 90)
- **IPI:** Cálculo com base no NCM, com observações fiscais para Simples Nacional
- **PIS/COFINS:** Tratamento por CST com bases de cálculo específicas
- **Substituição Tributária:** Cálculo de IVA e ICMS-ST quando aplicável

### Regimes Tributários
- **Regime Normal:** Cálculos completos de todos os tributos
- **Simples Nacional:** Ajustes automáticos de CST e alíquotas específicas
- **Importação:** Tratamento diferenciado para empresas importadoras

## 🎨 Saídas Geradas

| Saída | Descrição | Tipo |
|-------|-----------|------|
| eaa0103 | Item do documento com cálculos atualizados | Eaa0103 |

## 🔧 Dependências

**Bibliotecas:**
- `multiorm` - Persistência e consultas
- `sam.dicdados` - Definições de tipos de fórmula
- `sam.model.entities` - Entidades do sistema

**Entidades Relacionadas:**
- Todas entidades AA, AB, EA do pacote `sam.model.entities`

## 📝 Observações Técnicas

- **Processamento:** Síncrono, executado para cada item do documento
- **Validações:** Utiliza `ValidacaoException` para dados inconsistentes
- **Performance:** Carregamento otimizado de entidades relacionadas
- **Flexibilidade:** Campos livres (JSON) permitem extensibilidade

### Tratamentos Específicos
- **Material de Uso e Consumo:** Ajuste automático de CST para 90 e CFOP específico
- **Observações Fiscais de IPI:** Geradas automaticamente para operações específicas
- **Reduções de Base de Cálculo:** Aplicadas conforme configurações do item
- **Custo de Aquisição:** Calculado considerando todos os tributos e descontos

### Campos Livres (JSON) Utilizados
- `ipi_obs`: Observação fiscal de IPI
- `custo_aquisicao`: Custo total de aquisição
- `tx_iva_st`: Percentual de IVA para substituição tributária
- `icm_reduc_bc`: Percentual de redução de base de cálculo do ICMS